...
if 'pythonanywhere' in os.environ.get('PYTHONANYWHERE_DOMAIN', ''):
    # Используем временную директорию для matplotlib
...
def plot():
    ...
    for i, key in enumerate(keys_part1):
        ...
        ax1.plot(t_span_plot, sol[:, index],
                label=f"X{subscripts[index]}: {v0[key].name}",
                color=colors[index], linewidth=2)
        ax1.text(t_span_plot[-1] + 0.02, sol[-1, index], f"X{subscripts[index]}", 
    ...
    for i, key in enumerate(keys_part2):
        index = keys.index(key)
        ax2.plot(t_span_plot, sol[:, index],
                label=f"X{subscripts[index]}: {v0[key].name}",
                color=colors[index], linewidth=2)
        ax2.text(t_span_plot[-1] + 0.02, sol[-1, index], f"X{subscripts[index]}", 
                fontsize=14, fontweight='bold', color=colors[index],
                ha='left', va='center',
                bbox=dict(boxstyle='round,pad=0.2', facecolor='white', alpha=0.8))
